...
const blogPrisma = prisma as unknown as {
  blogPost: {
    findMany: (args: Record<string, unknown>) => Promise<Array<{ slug: string; updatedAt: Date }>>;
  };
};
...
export async function GET() {
  ...
  const blogs = await blogPrisma.blogPost.findMany({
  ...
  const urls = [
    ...
    ...blogs.map((blog: { slug: string; updatedAt: Date }) => buildUrl(`/blogs/${blog.slug}`, blog.updatedAt, "weekly", "0.6")),
  ].join("");
